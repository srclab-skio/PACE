...
% data_table = readtable('global_14Aug25_MODIS.csv', 'VariableNamingRule', 'preserve');
% data_table = readtable('global_18Aug25_PACE_bands_TSS_LT200.csv', 'VariableNamingRule', 'preserve');
data_table = readtable('Simul_RLw_Nechad_et_al_2015_PACE_Bands_only.csv', 'VariableNamingRule', 'preserve');
tss = data_table.TSS;  % TSS values (mg/L)
rrs_data = table2array(data_table(:, 2:end));  % Rrs spectra (rows: samples, cols: wavelengths)
num_samples = size(rrs_data, 1);
...
% Save the model
save('PACE_rf_model_best_Simulated.mat', 'rf_model_best', 'wavelengths');
...
% Save the figure
saveas(gcf, 'Simulated_feature_importance_best_2nd_Der_rrs.png');
...
% Save high-quality figure (matching figure 12 settings)
set(gcf, 'Position', [100, 100, 800, 700]);
saveas(gcf, 'Simulated_scatter_rf_deriv.png');
%saveas(gcf, 'scatter_rf_deriv.pdf');  % Vector format for publications
...
%% With Raw RRS without derivatives
%% Random Forest Model Using Raw Rrs Spectra
% This script trains a Random Forest model using raw Remote Sensing Reflectance (Rrs) data
% instead of second derivatives. All other steps are kept similar to the original code for
% consistency, including data loading, splitting, model training, evaluation, feature importance
...
% No second derivative computation - use raw Rrs as features
features = rrs_data;
...
% Train optimized Random Forest model with best hyperparameters
rf_model_best = TreeBagger(213, X_train, y_train, ...  % Optimized: 213 trees (note: original had 104, but keeping as per code)
    'Method', 'regression', ...
    'OOBPredictorImportance', 'on', ...
    'MinLeafSize', 4);  % Optimized: MinLeafSize=4
            
% Save the model with name indicating raw Rrs
save('_SimulateD_PACE_rf_model_raw.mat', 'rf_model_best', 'wavelengths');
...
% Display metrics
fprintf('Optimized Random Forest Model (Raw Rrs, RS=49, NumTrees=213, MinLeafSize=4):\n');
fprintf('R²: %.4f, RMSE: %.4f mg/L\n', metrics.R2, metrics.RMSE);
...
% Subplot 1: Bar plot of top 50 important features (adjusted to 50 as in original)
subplot(1, 2, 1);
barh(sorted_imp(1:min(50, num_wl)), 'FaceColor', [0.2, 0.6, 0.8]);
set(gca, 'YTick', 1:min(50, num_wl));
set(gca, 'YTickLabel', wavelengths(imp_idx(1:min(50, num_wl))));
ylabel('Wavelength (nm)');
xlabel('Feature Importance');
title('Top 50 Important Wavelengths (Raw Rrs)');
grid on;
set(gca, 'XAxisLocation', 'top');
set(gca, 'YDir', 'reverse');  % Highest importance at top
...
% Highlight top 20 wavelengths (adjusted to 20 as in original plot)
...
title('Feature Importance Spectrum (Raw Rrs)');
grid on;
legend('All wavelengths', 'Top 20 wavelengths', 'Location', 'best');

% Add text annotations for top 20 wavelengths
for i = 1:min(20, num_wl)
...
% Add overall title
sgtitle('RF Feature Importance - Raw Rrs Data', ...
        'FontSize', 14, 'FontWeight', 'bold');

% Save the figure
saveas(gcf, 'Simulated_feature_importance_raw_rrs.png');
...
% Scatter plot for Raw Rrs model
figure(15); 
hold on;
...
% Calculate metrics
r2_raw = 1 - sum((y_test - y_pred).^2) / sum((y_test - mean(y_test)).^2);
rmse_raw = sqrt(mean((y_test - y_pred).^2));

% Combine metrics and statistics in one text box
combined_str_raw = {sprintf('R² = %.3f', r2_raw), ...
                    sprintf('RMSE = %.2f mg/L', rmse_raw), ...
                    sprintf('n = %d', length(y_test)), ...
                    sprintf('Bias = %.2f mg/L', mean(y_pred - y_test))};

text_box_raw = annotation('textbox', [0.15, 0.75, 0.2, 0.12], 'String', combined_str_raw, ...
    'FitBoxToText', 'on', 'BackgroundColor', 'white', 'EdgeColor', 'black', ...
    'FontSize', 12, 'FontWeight', 'bold', 'Margin', 5, ...
    'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left');
...
p_raw = polyfit(y_test, y_pred, 1);
regress_line_raw = polyval(p_raw, xlim);
plot(xlim, regress_line_raw, 'k-', 'LineWidth', 1.5, 'LineStyle', ':');

% Legend
slope_raw = p_raw(1);
intercept_raw = p_raw(2);
legend({'Data Points', '1:1 Line', sprintf('Regression: y = %.2fx + %.2f', slope_raw, intercept_raw)}, ...
       'Location', 'southeast');
...
% Save high-quality figure
set(gcf, 'Position', [100, 100, 800, 700]);
saveas(gcf, 'Simulated_scatter_rf_raw.png');

% Display metrics in command window
fprintf('\nRandom Forest (Raw Rrs) Performance:\n');
fprintf('R²: %.4f\n', r2_raw);
fprintf('RMSE: %.4f mg/L\n', rmse_raw);
fprintf('Bias: %.4f mg/L\n', mean(y_pred - y_test));
fprintf('Sample size: %d\n', length(y_test));

%% 2nd der with more weights for 20 features
% Random Forest Model Using Second Derivatives with Weighted Top Features
% This script trains a Random Forest model using second derivatives of Rrs data, similar to the
% original code. After initial training, it identifies the top 20 important features and duplicates
% them in the feature set to give them more weight (appearing twice, effectively doubling their
% presence for higher selection probability in tree splits). Then, it retrains the model on the
% augmented data. All other steps are kept similar, with adjusted names for saving.
...
rf_model_initial = TreeBagger(213, X_train_initial, y_train, ...
    'Method', 'regression', ...
    'OOBPredictorImportance', 'on', ...
    'MinLeafSize', 4);
...
% Augment the feature set by duplicating the top 20 features (to add weight)
% This makes top features appear twice, increasing their selection probability
second_deriv_aug = [second_deriv, second_deriv(:, top20_idx)];

% Augment wavelengths for plotting (duplicates labeled as 'wl (dup)')
wavelengths_aug = [wavelengths, arrayfun(@(x) x, wavelengths(top20_idx))];  % Keep same values for duplicates

% Update train/test sets with augmented features
X_train = second_deriv_aug(train_idx, :);
X_test = second_deriv_aug(test_idx, :);

% Retrain optimized Random Forest model on augmented data
rf_model_weighted = TreeBagger(213, X_train, y_train, ...
    'Method', 'regression', ...
    'OOBPredictorImportance', 'on', ...
    'MinLeafSize', 4);
            
% Save the weighted model with appropriate name
save('PACE_rf_model_2nd_deriv_weighted.mat', 'rf_model_weighted', 'wavelengths_aug');

% Predict on test set
y_pred = predict(rf_model_weighted, X_test);
...
% Display metrics
fprintf('Optimized Random Forest Model (2nd Derivatives Weighted, RS=49, NumTrees=213, MinLeafSize=4):\n');
fprintf('R²: %.4f, RMSE: %.4f mg/L\n', metrics.R2, metrics.RMSE);
...
importances_aug = rf_model_weighted.OOBPermutedPredictorDeltaError;
...
for i = 1:min(50, length(wavelengths_aug))
    fprintf('Wavelength %d nm: Importance %.4f\n', wavelengths_aug(imp_idx_aug(i)), sorted_imp_aug(i));
end
...
set(gca, 'YTickLabel', wavelengths_aug(imp_idx_aug(1:min(50, length(wavelengths_aug)))));
ylabel('Wavelength (nm)');
xlabel('Feature Importance');
...
for i = 1:20
    text(top20_idx_aug(i), importances_aug(top20_idx_aug(i)) + 0.1, ...
         sprintf('%d nm', wavelengths_aug(top20_idx_aug(i))), ...
         'HorizontalAlignment', 'center', 'FontWeight', 'bold');
end
...
fprintf('Max importance: %.4f (at %d nm)\n', max(importances_aug), wavelengths_aug(imp_idx_aug(1)));
fprintf('Min importance: %.4f (at %d nm)\n', min(importances_aug), wavelengths_aug(imp_idx_aug(end)));
...
fprintf('Training Set Size: %d\n', sum(train_idx));
fprintf('Test Set Size: %d\n', sum(test_idx));
fprintf('Out-of-Bag Error: %.4f\n', mean(oobError(rf_model_weighted)));
...
fprintf('Bias: %.4f mg/L\n', mean(y_pred - y_test));
fprintf('Sample size: %d\n', length(y_test));
%% Adding Modified Nechad as a parameter
...
wavelengths_aug = [wavelengths, wavelengths(top20_idx), wavelengths(top20_idx), zeros(1, 5)];
...
fprintf('\nTop 50 Wavelengths by Importance (Random Forest, 2nd Derivatives Weighted):\n');
for i = 1:min(50, length(wavelengths_aug))
    if wavelengths_aug(imp_idx_aug(i)) == 0
...
ylabel('Feature');
xlabel('Feature Importance');
title('Top 50 Important Features (2nd Derivatives Weighted)');
grid on;
set(gca, 'XAxisLocation', 'top');
set(gca, 'YDir', 'reverse');  % Highest importance at top
...
% Add text annotations for top 20 (using wavelengths_aug or 'xtra')
for i = 1:20
